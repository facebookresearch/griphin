...
WORKER_NAME = 'worker{}'
FILE_PATH = os.path.join(get_data_path(), 'hz-ogbn-product-p{}-pt'.format(NUM_MACHINES))
...
def fetch_neighbor(rrefs):
    ...
    max_local_id = 500000  # make sure every shard has num_core_nodes larger than this value
...
def run(rank):
    ...
    if rank == 0:
        ...
        for machine_rank in range(NUM_MACHINES):
            ...
            rrefs.append(remote(info, GraphShard, args=(machine_rank, FILE_PATH)))
