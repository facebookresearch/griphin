...
if not osp.isdir(osp.join(args.path, args.dataset)):
    os.makedirs(osp.join(args.path, args.dataset))

og, _ = DglNodePropPredDataset(name=args.dataset, root=args.dgl_data_path)[0]
...
dgl.save_graphs(osp.join(args.path, args.dataset, 'dgl_data_processed'), g)
