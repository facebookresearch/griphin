...
parser.add_argument('--path', type=str, default='data/ogbn-products-p4', help='output_path')
parser.add_argument('--data_path', type=str, default='/data/gangda/graph_engine')
...
#     (og,), _ = dgl.load_graphs(os.path.join(args.dgl_data_path, args.data, 'dgl_data_processed'))
# else:
#     og, _ = DglNodePropPredDataset(name=args.data, root=args.dgl_data_path)[0]
# g = dgl.to_bidirected(og)
# g.edata['w'] = torch.rand((g.num_edges()))

(g,), _ = dgl.load_graphs(os.path.join(args.data_path, args.data, 'dgl_data_processed'))
...
to_file(args.path, 'metis_partitions.pt', parts)
to_file(args.path, 'local_id_mapping.pt', globalid_to_localid)
to_file(args.path, 'shard_id_mapping.pt', globalid_to_shardid)
to_file(args.path, 'dgl_edge_index.pt', g_indices)
to_file(args.path, 'dgl_edge_weights.pt', g.edata['w'])
...
for i in range(len(parts)):
    ...
    to_file(args.path, 'p{}_indptr.pt'.format(i), csr_indptr.to(EDGE_ID_TYPE))
    ...
    to_file(args.path, 'p{}_indices_node_id.pt'.format(i), csr_indices_engine_localid.to(VERTEX_ID_TYPE))
    to_file(args.path, 'p{}_indices_shard_id.pt'.format(i), csr_indices_engine_shardid.to(SHARD_ID_TYPE))
    ...
    to_file(args.path, 'p{}_indices_edge_weight.pt'.format(i), csr_indices_weights.to(WEIGHT_TYPE))
    to_file(args.path, 'p{}_indices_weighted_degree.pt'.format(i), csr_indices_weighted_degree.to(WEIGHT_TYPE))
